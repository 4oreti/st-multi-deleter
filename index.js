    ...
    input.onchange = async (e) => {
        ...
        for (const line of lines) {
            try {
                ...
                // 酒馆的 .jsonl 第一行通常是包含 user_name 的元数据，只有包含 .mes 的才是真正的聊天
                if (obj.mes !== undefined) {
                    importedMessages.push(obj);
                }
            } catch(err) {
                console.error("解析 JSONL 出错:", line, err);
            }
        }

        if (importedMessages.length === 0) {
            return toastr.warning("在这个文件中没有找到有效的聊天信息 o^o");
        }

        if (!confirm(`在文件中发现了 ${importedMessages.length} 条信息！\n确定要把它们拼接到当前聊天的最末尾吗？`)) {
            return;
        }
        ...
        // ★ 核心魔法：把新导入的信息直接缝合到数组最尾部
        cArray.push(...importedMessages);
        ...
        toastr.success(`已将 ${importedMessages.length} 条信息合并到当前聊天中`);
        isProcessing = false;
        toggleMode(); // 导入完毕后自动退出选择模式
    };
    input.click();
}
...
function showControlPanel() {
    ...
    panel.innerHTML = `
        ...
        <div style="display: flex; justify-content: center; flex-wrap: wrap; gap: 8px;">
            <!-- ★ 这里新增了导入按钮 -->
            <button id="md-btn-import" class="md-theme-btn" style="background: var(--SmartThemeBlurTintColor);" title="选择一个.jsonl文件，将里面的聊天记录追加到当前页面末尾"><i class="fa-solid fa-file-import"></i> 导入合并</button>
    ...
    // 绑定导入事件
    $('#md-btn-import').on('click', importAndAppendChat);
            ...
            <div style="position: absolute; bottom: 0; left: 0; right: 0; height: 60px; padding: 0 15px; border-top: 1px solid var(--SmartThemeBorderColor); background: var(--SmartThemeBlurTintColor); display: flex; justify-content: space-between; align-items: center; z-index: 10;">
                ...
                <div style="display:flex; gap:8px;">
                    ...
                    <button id="md-modal-move" class="md-theme-btn" style="background: #8e44ad; color: white !important; border:none; padding: 6px 12px; font-weight:bold; cursor:pointer;">
                        <i class="fa-solid fa-truck-fast"></i> 搬出
                    </button>
...
async function executeDelete(isMove = false) {
    ...
    if (successCount > 0) {
        if (isMove) toastr.success(`成功搬出 ${successCount} 条信息 你可以去其他聊天使用【导入合并】啦 `, "搬出完成", {timeOut: 8000});
        else toastr.success(`成功删除了 ${successCount} 条信息`);
    } else {
...
jQuery(() => {
    const checkBtn = setInterval(() => {
        ...
        if (bar && !document.getElementById('multi-delete-open-btn')) {
            ...
            btn.innerHTML = '<div class="fa-solid fa-list-check"></div><div>批量管理与搬家</div>'; // 名字升级了！
